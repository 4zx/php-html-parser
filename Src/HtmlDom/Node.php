...
class Node
{
    ...
    function dump_node($echo = true)
    {
        ...
        if (isset($node->_[Config::get('HDOM_INFO_INNER')])) {
            $string .= "'" . $node->_[Config::get('HDOM_INFO_INNER')] . "'";
        } else {
            $string .= ' NULL ';
        }
    ...
    function innertext()
    {
        if (isset($this->_[Config::get('HDOM_INFO_INNER')])) {
            return $this->_[Config::get('HDOM_INFO_INNER')];
        }

        if (isset($this->_[Config::get('HDOM_INFO_TEXT')])) {
            return $this->dom->restore_noise($this->_[Config::get('HDOM_INFO_TEXT')]);
        }
    ...
    function outertext()
    {
        ...
        if (isset($this->_[Config::get('HDOM_INFO_OUTER')])) {
            return $this->_[Config::get('HDOM_INFO_OUTER')];
        }
        ...
        if ($this->dom && $this->dom->nodes[$this->_[Config::get('HDOM_INFO_BEGIN')]]) {
            $ret = $this->dom->nodes[$this->_[Config::get('HDOM_INFO_BEGIN')]]->makeup();
        }

        if (isset($this->_[Config::get('HDOM_INFO_INNER')])) {
            // todo: <br> should either never have HDOM_INFO_INNER or always
            if ($this->tag !== 'br') {
                $ret .= $this->_[Config::get('HDOM_INFO_INNER')];
            }
        } elseif ($this->nodes) {
            foreach ($this->nodes as $n) {
        ...
        if (isset($this->_[Config::get('HDOM_INFO_END')]) && $this->_[Config::get('HDOM_INFO_END')] != 0) {
            $ret .= '</' . $this->tag . '>';
        }
    ...
    function text()
    {
        ...
        switch ($this->nodetype) {
            case Config::get('HDOM_TYPE_TEXT'):
                return $this->dom->restore_noise($this->_[Config::get('HDOM_INFO_TEXT')]);
            case Config::get('HDOM_TYPE_COMMENT'):
                return '';
            case Config::get('HDOM_TYPE_UNKNOWN'):
                return '';
        }
    ...
    function makeup()
    {
        ...
        foreach ($this->attr as $key => $val) {
            ...
            $ret .= $this->_[Config::get('HDOM_INFO_SPACE')][$i][0];

            //no value attr: nowrap, checked selected...
            if ($val === true) {
                $ret .= $key;
            } else {
                switch ($this->_[Config::get('HDOM_INFO_QUOTE')][$i]) {
                    case Config::get('HDOM_QUOTE_DOUBLE'):
                        $quote = '"';
                        break;
                    case Config::get('HDOM_QUOTE_SINGLE'):
                        $quote = '\'';
                        break;
                    default:
                        $quote = '';
                ...
                $ret .= $key
                    . $this->_[Config::get('HDOM_INFO_SPACE')][$i][1]
                    . '='
                    . $this->_[Config::get('HDOM_INFO_SPACE')][$i][2]
                    . $quote
                    . $val
                    . $quote;
            }
    ...
    function find($selector, $idx = null, $lowercase = false)
    {
        ...
        for ($c = 0; $c < $count; ++$c) {
            ...
            if (!isset($this->_[Config::get('HDOM_INFO_BEGIN')])) {
                return array();
            }

            $head = array($this->_[Config::get('HDOM_INFO_BEGIN')] => 1);
            $cmd = ' '; // Combinator
    ...
    protected function seek($selector, &$ret, $parent_cmd, $lowercase = false)
    {
        ...
        if ($parent_cmd === ' ') { // Descendant Combinator
            ...
            $end = (!empty($this->_[Config::get('HDOM_INFO_END')])) ? $this->_[Config::get('HDOM_INFO_END')] : 0;
            if ($end == 0) {
                $parent = $this->parent;
                while (!isset($parent->_[Config::get('HDOM_INFO_END')]) && $parent !== null) {
                    $end -= 1;
                    $parent = $parent->parent;
                }
                $end += $parent->_[Config::get('HDOM_INFO_END')];
            }

            // Get list of target nodes
            $nodes_start = $this->_[Config::get('HDOM_INFO_BEGIN')] + 1;
        ...
        foreach ($nodes as $node) {
            ...
            if ($pass) $ret[$node->_[Config::get('HDOM_INFO_BEGIN')]] = 1;
            unset($node);
        }
    ...
    function __set($name, $value)
    {
        ...
        switch ($name) {
            case 'outertext':
                return $this->_[Config::get('HDOM_INFO_OUTER')] = $value;
            case 'innertext':
                if (isset($this->_[Config::get('HDOM_INFO_TEXT')])) {
                    return $this->_[Config::get('HDOM_INFO_TEXT')] = $value;
                }
                return $this->_[Config::get('HDOM_INFO_INNER')] = $value;
        }

        if (!isset($this->attr[$name])) {
            $this->_[Config::get('HDOM_INFO_SPACE')][] = array(' ', '', '');
            $this->_[Config::get('HDOM_INFO_QUOTE')][] = Config::get('HDOM_QUOTE_DOUBLE');
        }
