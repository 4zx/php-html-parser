...
class Parser
{
    ...
    function load(
        ...
        if ($options & Config::get('HDOM_SMARTY_AS_TEXT')) { // Strip Smarty scripts
        ...
        $this->parse();
        // end
        $this->root->_[Config::get('HDOM_INFO_END')] = $this->cursor;
        $this->parse_charset();
    ...
    protected function prepare(
        ...
        $this->root->tag = 'root';
        $this->root->_[Config::get('HDOM_INFO_BEGIN')] = -1;
        $this->root->nodetype = Config::get('HDOM_TYPE_ROOT');
    ...
    protected function parse()
    {
        while (true) {
            ...
            $node = new Node($this);
            ++$this->cursor;
            $node->_[Config::get('HDOM_INFO_TEXT')] = $s;
    ...
    protected function read_tag()
    {
        ...
        if ($this->char === '/') {
            ...
            if ($parent_lower !== $tag_lower) {
                ...
                if (
                    ...
                    $this->parent->_[Config::get('HDOM_INFO_END')] = 0;
                    ...
                    if (strtolower($this->parent->tag) !== $tag_lower) {
                        ...
                        $this->parent->_[Config::get('HDOM_INFO_END')] = $this->cursor;
                    ...
                    $this->parent->_[Config::get('HDOM_INFO_END')] = 0; // No end tag
                    $org_parent = $this->parent;
        ...
        // start tag
        $node = new Node($this);
        $node->_[Config::get('HDOM_INFO_BEGIN')] = $this->cursor;
        ++$this->cursor;
        ...
        if (isset($tag[0]) && $tag[0] === '!') {
            $node->_[Config::get('HDOM_INFO_TEXT')] = '<' . $tag . $this->copy_until_char('>');

            if (isset($tag[2]) && $tag[1] === '-' && $tag[2] === '-') { // Comment ("<!--")
                $node->nodetype = Config::get('HDOM_TYPE_COMMENT');
                $node->tag = 'comment';
            } else { // Could be doctype or CDATA but we don't care
                $node->nodetype = Config::get('HDOM_TYPE_UNKNOWN');
                $node->tag = 'unknown';
            }

            if ($this->char === '>') {
                $node->_[Config::get('HDOM_INFO_TEXT')] .= '>';
            }
        ...
        if ($pos = strpos($tag, '<') !== false) {
            $tag = '<' . substr($tag, 0, -1);
            $node->_[Config::get('HDOM_INFO_TEXT')] = $tag;
            $this->link_nodes($node, false);
        ...
        if (!preg_match('/^\w[\w:-]*$/', $tag)) {
            $node->_[Config::get('HDOM_INFO_TEXT')] = '<' . $tag . $this->copy_until('<>');
        ...
        $node->nodetype = Config::get('HDOM_TYPE_ELEMENT');
        $tag_lower = strtolower($tag);
        ...
        do {
            ...
            if ($this->pos >= $this->size - 1 && $this->char !== '>') {
                $node->nodetype = Config::get('HDOM_TYPE_TEXT');
                $node->_[Config::get('HDOM_INFO_END')] = 0;
                $node->_[Config::get('HDOM_INFO_TEXT')] = '<' . $tag . $space[0] . $name;
                $node->tag = 'text';
                $this->link_nodes($node, false);
                return true;
            }
            ...
            if ($this->doc[$this->pos - 1] == '<') {
                ...
                $node->attr = array();
                $node->_[Config::get('HDOM_INFO_END')] = 0;
                $node->_[Config::get('HDOM_INFO_TEXT')] = substr(
                    $this->doc,
                    $begin_tag_pos,
            ...
            if ($name !== '/' && $name !== '') { // this is a attribute name
                ...
                if ($this->char === '=') { // attribute with value
                    ...
                    $node->_[Config::get('HDOM_INFO_QUOTE')][] = Config::get('HDOM_QUOTE_NO');
                    $node->attr[$name] = true;
                    if ($this->char != '>') {
                ...
                $node->_[Config::get('HDOM_INFO_SPACE')][] = $space;
        ...
        if ($node->tag === 'br') {
            $node->_[Config::get('HDOM_INFO_INNER')] = $this->default_br_text;
        }
    ...
    protected function parse_attr($node, $name, &$space)
    {
        ...
        switch ($this->char) {
            case '"':
                $quote_type = Config::get('HDOM_QUOTE_DOUBLE');
                $this->char = (++$this->pos < $this->size) ? $this->doc[$this->pos] : null; // next
                $value = $this->copy_until_char('"');
                $this->char = (++$this->pos < $this->size) ? $this->doc[$this->pos] : null; // next
                break;
            case '\'':
                $quote_type = Config::get('HDOM_QUOTE_SINGLE');
                $this->char = (++$this->pos < $this->size) ? $this->doc[$this->pos] : null; // next
                $value = $this->copy_until_char('\'');
                $this->char = (++$this->pos < $this->size) ? $this->doc[$this->pos] : null; // next
                break;
            default:
                $quote_type = Config::get('HDOM_QUOTE_NO');
        ...
        if (!$is_duplicate) {
            $node->_[Config::get('HDOM_INFO_QUOTE')][] = $quote_type;
